...
export class AgVirtualScrollComponent
  implements OnInit, AfterViewInit, AfterContentInit, OnChanges, OnDestroy
{
  @ViewChild('itemsContainer', { static: true })
  private itemsContainerElRef: ElementRef<HTMLElement>;

  @ContentChildren(AgVsItemComponent)
  private queryVsItems: QueryList<AgVsItemComponent>;
  ...
  private set indexCurrentSticky(value: number) {
    ...
    if (!currentIsPrev && value >= 0) {
      ...
      if (!currentIsPrev) this.indexNextSticky = -1;

      if (this.currentStickyItem) this.currentStickyItem.comp.isSticked = false;
  ...
  protected tBodyEl: HTMLHeadElement;
  protected tableEl: HTMLHeadElement;
  ...
  private get itemsNoSticky() {
    return this.currentStickyItem
      ? this.items.filter(
          (item) => this.originalItems[this.currentStickyItem.index] !== item,
        )
      : this.items;
  }
  public get vsItems() {
  ...
  public get numberItemsRendered(): number {
  ...
  private get heightIsInPixels() {
    return this.height?.endsWith('px') || /^([\s\d]+)$/g.test(this.height);
  }
  private get heightValue() {
    if (this.heightIsInPixels) {
      const onlyNumber = (() => {
        const matched = this.height?.match(
          /[-]{0,1}[\d]*[,]?[\d]*[.]{0,1}[\d]+/g,
        );
  ...
  ngAfterViewInit() {
    this.queryVsItems.changes.pipe(takeUntil(this.destroy$)).subscribe(() => {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if ('minRowHeight' in changes) {
      if (typeof this.minRowHeight === 'string') {
        if (parseFloat(this.minRowHeight))
          ...
          console.warn(
            'The [min-row-height] @Input is invalid, the value must be of type "number".',
          );
    ...
    if (this.isInitialized && 'originalItems' in changes) {
      ...
      if (this.currentAndPrevItemsAreDiff()) {
        this.previousItemsHeight = new Array(this.originalItems.length).fill(
          null,
        );
        ...
        if (this.originalItems.length > this.prevOriginalItems.length)
          this.previousItemsHeight = this.previousItemsHeight.concat(
            new Array(
              this.originalItems.length - this.prevOriginalItems.length,
            ).fill(null),
          );
      }
    }
  ...
  private currentAndPrevItemsAreDiff() {
    if (this.originalItems.length >= this.prevOriginalItems.length) {
      ...
      for (let i = begin; i <= end; i++) {
        if (this.originalItems[i] !== this.prevOriginalItems[i]) return true;
      }
  ...
  private registerCurrentItemsHeight() {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      this.previousItemsHeight[realIndex] =
  ...
  private prepareDataVirtualScroll() {
    ...
    if (!this.el.style.maxHeight) {
      console.warn(
        `The [height] @Input is invalid or wasn't defined. This @Input must be like this: height="500px".`,
      );
      return;
    }
    if (!this.heightValue) {
    ...
    this.endIndex =
      Math.min(this.startIndex + qnttyCanRender, this.originalItems.length) - 1;

    if (
      this.indexCurrentSticky >= 0 &&
      (this.startIndex > this.indexCurrentSticky ||
        this.endIndex < this.indexCurrentSticky)
    ) {
      if (this.currentStickyItem) this.currentStickyItem.outside = true;
      this.items = [
        ...this.originalItems.slice(this.startIndex, this.endIndex + 1),
        this.originalItems[this.indexCurrentSticky],
      ];
    } else {
      if (this.currentStickyItem) this.currentStickyItem.outside = false;
      ...
      if (childrens[0].tagName.toUpperCase() === 'TABLE') {
        ...
        if (childrens.length > 0) {
          if (childrens[0].tagName.toUpperCase() === 'TBODY')
            childrens = childrens[0].children;
          else childrens = childrens[1].children;
        }
      }
    }
  ...
  private checkIsTable() {
    let children = this.itemsContainerEl.children;
    if (children.length > 0) {
      const el = children[0] as HTMLElement;
      if (el.tagName.toUpperCase() === 'TABLE') {
        el.style.transform = `translateY(${Math.abs(this.paddingTop)}px)`;
        const thead = el.children[0] as HTMLElement;
        if (thead) {
          const top = Math.max(this.currentScroll - Math.abs(this.paddingTop), 0);
            ...
            if (this.currentStickIsEnded(up)) {
              if (!up) {
                ...
                if (this.indexPrevSticky >= 0) {
                  ...
                  if (this.indexCurrentSticky >= 0)
                    this.indexNextSticky = this.getIndexNextSticky(up);
                  else this.indexNextSticky = null;
                }
              }
            }
          } else {
  ...
  private findCurrentStickyByIndex(afterPrev: boolean = false): void {
    ...
    if (
      diffMaxItemsRender > 0 &&
      !this.vsItems.some(
        (vsItem, vsIndex) =>
          this.indexCurrentSticky === this.startIndex + vsIndex,
      )
    ) {
      vsIndex = lastVsIndex;
      let vsItem = this.vsItems[lastVsIndex];
      let index = this.indexCurrentSticky;
      let offsetTop = this.previousItemsHeight
        .slice(0, index)
        .reduce((prev, curr) => prev + (curr ? curr : this.minRowHeight), 0);
      ...
      for (let vsItem of this.vsItems) {
        ...
        if (this.indexCurrentSticky === index) {
          let offsetTop = this.previousItemsHeight
            .slice(0, index)
            .reduce(
              (prev, curr) => prev + (curr ? curr : this.minRowHeight),
              0,
            );
          vsItem.isSticked = true;
          this.currentStickyItem = new StickyItem({
    ...
    if (afterPrev && this.currentStickyItem) {
      ...
      let offsetBottom =
        this.paddingTop +
        currentHeight +
        Math.abs(this.el.scrollTop - this.paddingTop);
      let offsetTopNext =
        this.indexNextSticky >= 0
          ? this.previousItemsHeight
              .slice(0, this.indexNextSticky)
              .reduce(
                (prev, curr) => prev + (curr ? curr : this.minRowHeight),
                0,
              )
      ...
      if (offsetTopNext !== null && offsetBottom >= offsetTopNext) {
        ...
        } else this.currentStickyItem.diffTop = newDiffTop;
      } else this.currentStickyItem.diffTop = 0;
    }
  }

  private setPrevAsCurrentSticky() {
    let currentSticked =
      this.currentStickyItem && this.currentStickyItem.comp.sticky;

    if (currentSticked) this.indexNextSticky = this.indexCurrentSticky;
  ...
  private getIndexNextSticky(up: boolean) {
    if (this.indexCurrentSticky >= 0) {
      ...
      for (let vsItem of this.vsItems.slice(0, this.numberItemsCanRender())) {
        ...
        if (vsItem.sticky && index > this.indexCurrentSticky) return index;
  ...
  private currentStickIsEnded(up: boolean) {
    ...
    if (!up || this.currentStickyItem.diffTop > 0) {
      ...
      let offsetBottom =
        this.paddingTop + Math.abs(this.el.scrollTop - this.paddingTop);
  ...
  private updateVsItems() {
    return new Observable<void>((subscriber) => {
      ...
      let interval = setInterval(() => {
        let diffMaxItemsRender =
          this.vsItems.length - this.numberItemsCanRender();
  ...
  private onStickyComponentChanged(vsItem: AgVsItemComponent, index: number) {
    if (index === this.indexCurrentSticky) {
      ...
      (this.indexCurrentSticky !== -1 && index < this.indexCurrentSticky) ||
      index === this.indexPrevSticky
    ) {
      if (vsItem.sticky) this.indexPrevSticky = index;
      else
        this.indexesPrevStick = this.indexesPrevStick.filter(
          (indexPrev) => indexPrev !== index,
        );
    } else if (
      (this.indexCurrentSticky !== -1 && index > this.indexCurrentSticky) ||
      index === this.indexNextSticky
    ) {
      if (
        vsItem.sticky &&
        (this.indexNextSticky === -1 || index < this.indexNextSticky)
      )
        this.indexNextSticky = index;
      else if (!vsItem.sticky) this.indexNextSticky = -1;
    } else return;
